...
int ICMP_Ping_Mod::exec(Target *tg, OS_Matrix *os) {
    ...
    if (do_icmp_ping(tg)) {
        xprobe_debug(XPROBE_DEBUG_MODULES, "--%s module XPROBE MATCH YES against: %s\n"
                     , get_name(), inet_ntoa(tg->get_addr()));
        os->add_result(get_id(), 1, XPROBE_MATCH_YES);
    } else {
        xprobe_debug(XPROBE_DEBUG_MODULES, "--%s module XPROBE MATCH NO against: %s\n"
                     , get_name(), inet_ntoa(tg->get_addr()));
...
/* our lamyer randomizer ;-p */
    ...
    xprobe_debug(XPROBE_DEBUG_MODULES, "--icmp ping query on %s interface for target %s\n",
                 tg->get_interface(), inet_ntoa(tg->get_addr()));
    ...
    while (!done) {
        ...
        xprobe_debug(XPROBE_DEBUG_MODULES, "--icmp ping: %i got packet from %x type %i icmp_id %i\n",
                     ret, sn.get_src(), sn.get_type(), sn.get_icmpId());
        /* packet response */
